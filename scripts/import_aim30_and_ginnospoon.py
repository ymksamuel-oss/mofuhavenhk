...
MANIFEST_PATH = ROOT / "aim30_and_ginnospoon_stripe_manifest.json"
...
def product_metadata(item: dict, pricing_rule: dict) -> dict[str, str]:
    is_dry_food = item["subcategory"] == "dry-food"
    specs_zh = item.get("specs_zh", item.get("option_label_zh", "日本包裝規格"))
    specs_en = item.get("specs_en", item.get("option_label_en", "Japanese retail pack"))
    return {
        ...
        "category": "cats",
        "category_zh": "貓咪商品",
        "subcategory": "貓乾糧" if is_dry_food else "貓咪小食",
        "product_type": "cat dry food" if is_dry_food else "cat treats",
        ...
        "specs_zh": specs_zh,
        "specs_en": specs_en,
        "life_stage_zh": item.get("life_stage_zh", "全齡貓"),
        "life_stage_en": item.get("life_stage_en", "All life stages"),
        "availability": "現貨",
        "availability_display_zh": f"現貨｜{specs_zh}｜日本包裝",
        "availability_display_en": f"In stock | {specs_en} | Japanese retail pack",
        "in_stock": "true",
...
def update_or_create_product(item: dict, pricing_rule: dict, index: int) -> dict:
    ...
    if existing:
        return request("POST", f"products/{existing['id']}", data=payload, key=f"aim30-ginnospoon-product-update-v1-{index}")
    return request("POST", "products", data=payload, key=f"aim30-ginnospoon-product-create-v1-{index}")
...
def update_or_create_price(product: dict, item: dict, pricing_rule: dict, index: int) -> tuple[dict, str]:
    ...
    price_data = {
        ...
        "metadata[variant_label_zh]": item.get("option_label_zh", item.get("specs_zh", "單一規格")),
        "metadata[variant_label_en]": item.get("option_label_en", item.get("specs_en", "Single pack")),
        "metadata[cost_cny]": f"{Decimal(str(item['cost_cny'])).quantize(Decimal('0.01'))}",
        "metadata[unrounded_retail_hkd]": f"{Decimal(str(item.get('unrounded_retail_hkd', item['retail_hkd']))).quantize(Decimal('0.01'))}",
        "metadata[retail_pricing_rule]": "CNY×1.1654/(1-45%), upward .90",
        "metadata[compare_at_price_hkd]": "0",
    }
    return request("POST", "prices", data=price_data, key=f"aim30-ginnospoon-price-create-v1-{index}"), "created"
    ...
    for index, item in enumerate(mapping["products"], start=1):
        ...
        if product.get("default_price") != price["id"]:
            product = request(
                ...
                key=f"aim30-ginnospoon-default-price-v1-{index}",
            )
