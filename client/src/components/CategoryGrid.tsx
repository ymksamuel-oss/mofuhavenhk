...
export default function CategoryGrid() {
  return (
    <section aria-label="商品分類" className="relative overflow-hidden bg-[#F7F3EE] py-2 sm:py-3 md:py-8">
      <div className="pointer-events-none absolute left-0 top-1/3 h-72 w-72 rounded-full bg-white/20 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 right-0 h-96 w-96 rounded-full bg-[#B88A58]/10 blur-3xl" />
      ...
      <div className="container relative z-10">
          ...
          {catalogHierarchy.map((category) => {
            ...
            return (
              <a key={category.key} href={`/products?category=${category.key}`} className="group shrink-0 snap-start">
                <Card className="flex h-10 min-h-10 w-max cursor-pointer flex-row items-center justify-center gap-1.5 rounded-full border border-[#B88A58]/55 bg-[#FFFDF9] jp-card-shadow px-3 text-center shadow-[0_5px_16px_rgba(140,107,83,0.08)] backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[#A67C52] hover:bg-[#F7F3EE] hover:shadow-[0_8px_20px_rgba(140,107,83,0.15)]">
                  <div className="relative flex shrink-0 items-center justify-center rounded-full border border-[#B88A58]/35 bg-[#F7F3EE] p-1 transition-all duration-200 group-hover:bg-[#EAD2B9]">
                    <Icon className="relative z-10 h-4 w-4 text-[#736859]" />
                  </div>
                  <h3 className="mb-0 whitespace-nowrap text-xs font-semibold text-[#3E3A37] transition-colors group-hover:text-[#736859]">
                    {category.label}
                  </h3>
                </Card>
              </a>
          ...
          {catalogHierarchy.flatMap((category) => category.subCatalogs).map((subCatalog) => {
            ...
            return (
              <a key={subCatalog.key} href={`/products?category=${subCatalog.key}`} className="group shrink-0 snap-start">
                <Card className="flex h-8 min-h-8 w-max cursor-pointer flex-row items-center gap-1 rounded-full border border-[#B88A58]/35 bg-[#FFFDF9] jp-card-shadow px-2.5 text-center transition-all duration-200 hover:border-[#A67C52] hover:bg-[#F7F3EE]">
                  <Icon className="h-3.5 w-3.5 text-[#736859]" />
                  <span className="whitespace-nowrap text-[11px] font-medium text-[#3E3A37]">{subCatalog.label}</span>
                </Card>
              </a>
            );
          })}
        ...
        <div className="mt-2 hidden text-center md:mt-5 md:block">
          <Button asChild size="lg" className="rounded-full bg-[#A67C52] px-7 font-semibold text-white shadow-[0_8px_20px_rgba(140,107,83,0.18)] transition-all duration-300 hover:bg-[#736859] hover:shadow-[0_12px_26px_rgba(140,107,83,0.24)]">
