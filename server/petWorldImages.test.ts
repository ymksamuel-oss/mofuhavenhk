...
describe("pet world real photos and multi-image carousel", () => {
  ...
  it("includes multi-image carousel container and pagination dots in PetWorld.tsx", () => {
    ...
    expect(petWorldCode).toContain("relative aspect-[4/3] w-full overflow-hidden bg-white");
    expect(petWorldCode).toContain("horizontal-scroll flex w-full gap-2 overflow-x-auto border-t border-[#D3A87C]/15 bg-white p-2.5");
