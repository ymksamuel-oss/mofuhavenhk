...
- [ ] 檢查現有購物車結帳、Stripe Session 及手機首頁結構
- [ ] 建立香港本地收貨資料表格：收件人姓名、聯絡電話、送貨方式、順豐站／智能櫃點碼
- [ ] 表格提交後才建立 Stripe Checkout，並將收貨資料安全傳遞至付款 Session
- [ ] 停用 Stripe Express Checkout／Apple Pay／Link 頂部快捷按鈕，避免重複快捷付款入口
- [ ] 設定 Stripe 不再重複收集送貨地址，保留付款所需必要資料
- [ ] 清理手機首頁冗長品牌介紹、調試文字及系統標籤，保留搜尋／分類及商品快速曝光
- [ ] 驗證手機 Header 不超過 60px、表格驗證、結帳延後跳轉及付款錯誤處理
- [ ] 執行測試、保存 checkpoint 並部署
- [ ] 回報結帳與手機 UX 更新結果

## 最新結帳／分類列／手機首頁更新

- [ ] 檢查現有 Checkout、購物車、搜尋下方分類列及手機首頁結構
- [ ] 建立香港本地收貨資料表格：收件人姓名、聯絡電話、送貨方式、順豐站／智能櫃點碼
- [ ] 表格提交後才建立 Stripe Checkout，並避免 Stripe 重複收集送貨地址
- [ ] 停用 Express Checkout／Apple Pay／Link 頂部快捷按鈕
- [ ] 移除搜尋欄正下方重複的分類按鈕列，只保留上方分類按鈕
- [ ] 清理手機首頁冗長品牌介紹、調試文字及系統標籤，保留商品快速曝光
- [ ] 驗證手機 Header 不超過 60px、分類只出現一次、表格驗證及付款延後跳轉
- [ ] 執行測試、保存 checkpoint 並部署
- [ ] 回報最新結帳與手機 UX 更新結果
...
- [ ] 逐一驗證首頁及商品頁全部分類按鈕均能正確篩選對應商品或顯示正確空狀態
- [ ] 補充分類映射修正後的全分類自動化回歸測試與驗證紀錄

## 首頁 CategoryGrid 全量互動驗證

- [ ] 逐一驗證首頁 CategoryGrid 全部分類卡的點擊結果，確認每個分類都導向正確 `/products?category=...` 並顯示對應商品或正確空狀態
- [ ] 保存首頁與商品頁全分類前端互動驗證證據（截圖或自動化 UI 測試）
