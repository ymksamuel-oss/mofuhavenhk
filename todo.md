...
- [ ] 回溯 Git／歷史部署與原始資產資料夾，確認先前 Pet World 貓咪圖片的實際存放位置
- [ ] 逐一核對 12 個品種的前端 `image`／`images` 資料、受控資產路由與畫面呈現，定位所有失效或錯配來源
- [ ] 修復圖片資料來源與前端 `src`，只使用可驗證的原始品種圖片，禁止通用品種替代
- [ ] 加入資產可讀取與品種一對一映射回歸測試，重新打包並部署至正式網站
- [ ] 於正式 `/pet-world` 驗證無佔位符、圖片正常顯示且圖文相符
