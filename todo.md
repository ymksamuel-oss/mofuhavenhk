...
- [ ] 讀取原圖尺寸及逐格核對所有產品、尺寸、人民幣成本與變體關係
- [ ] 只保留價錢及款式關係清晰的產品，排除不清楚項目
- [ ] 為核准產品建立 1:1 米白 `#F7F1E6`、無簡體中文及無拼圖背景的產品圖片
- [ ] 以 `ceil_to_.90(CNY × 1.1654 × 1.76)` 計算港幣零售價並保存報告
- [ ] 建立 Stripe Products / Prices，寫入穩定變體鍵及圖片映射
- [ ] 將批次資料提交至 GitHub main，確認 Vercel production deployment
- [ ] 驗證產品頁價格、變體選擇、主圖切換及加入購物籃流程
- [ ] 整理暫用原始截圖及待日後清理的圖片清單

- [ ] 若同一產品的差異只屬顏色，未列出獨立價錢時沿用該產品已核實的相同 CNY 成本及 HKD 零售價；尺寸或配件組合不可自動沿用
