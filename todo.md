...
- [x] 驗證 `https://mofuhavenhk.com` 根網域目前受 GoDaddy 保護的 A 紀錄影響，首頁回應 HTTP 405，未能作為正式站點或一致轉址來源
- [x] 驗證根網域下的商品 API 回應 HTTP 404；確認所有店面與 API 流量必須使用 `https://www.mofuhavenhk.com`
- [x] 記錄最終策略：`www.mofuhavenhk.com` 為唯一 Vercel 正式 Production 網域（首頁及商品 API 均回應 HTTP 200）；根網域不再用作服務入口
...
- [x] 盤點 public／靜態資產與 Pet World 圖片資料來源，找出英國短毛貓等卡片回落至佔位符的原因
- [x] 修正 Pet World 圖片 URL／路由與容錯邏輯，維持嚴格品種對應
- [x] 加入圖片路徑回歸測試，確認正式可存取的真實圖片不會顯示「真實圖片準備中」
- [x] 重新打包、部署並在正式網站驗證寵物世界圖片顯示
...
## Pet World 原始圖片路徑回溯與重發佈

- [x] 回溯 Git／歷史部署與原始資產資料夾，確認先前 Pet World 貓咪圖片的實際存放位置
- [x] 逐一核對 12 個品種的前端 `image`／`images` 資料、受控資產路由與畫面呈現，定位所有失效或錯配來源
- [x] 修復圖片資料來源與前端 `src`，只使用可驗證的原始品種圖片，禁止通用品種替代
- [x] 加入資產可讀取與品種一對一映射回歸測試，重新打包並部署至正式網站
- [x] 於正式 `/pet-world` 驗證無佔位符、圖片正常顯示且圖文相符
...
## Pet World 三個品種六張真實圖片擴充

- [ ] 為英國短毛貓、美國短毛貓與布偶貓各搜尋並核對六張可公開使用、授權可追溯的真實照片
- [ ] 將十八張照片上傳至持久靜態儲存，更新三個品種的嚴格圖片映射及授權標註
- [ ] 驗證三個品種在桌面與手機版均呈現六張縮圖、主圖切換與正確圖片來源
- [ ] 移除三個品種公開畫面與公開資料中的原始圖片網址文字，只保留受控資產路徑及簡潔授權歸屬
