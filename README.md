...
Server 會讀取以下 Google Sheet CSV，按 `商品 ID` 對應現有 178 個商品，並一次過套用圖片、名稱、介紹、庫存及價錢。只要 Sheet 完整通過驗證，這五類欄位就以 Sheet 為唯一主源；程式內舊值只會在整份 Sheet 失效時作 fallback。
...
加入本批 12 款商品後，Google Sheet 必須補上相同 12 個商品 ID，令有效資料由 102 行增至 114 行；未補齊之前，完整覆蓋檢查會按設計退回 178 款靜態 catalog，避免新舊資料混合。
...
驗證涵蓋：178 筆產品、ID 唯一、必填欄位、中英文名稱、合理價格／原價、所有靜態 fallback 圖片存在、178/178 搜尋覆蓋、貓狗各 6 款投藥餵藥專用小食、Sheet 第二行中文表頭、五類欄位覆蓋、遠端圖片、空白介紹、價錢格式清理、不安全圖片路徑、重複 ID、fallback 不變性，以及伺服器訂單重建規則。
