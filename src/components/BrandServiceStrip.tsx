...
export function BrandServiceStrip({ placement = "top" }: { placement?: "top" | "catalog-bottom" }) {
  ...
  return (
    <aside className={`${placement === "catalog-bottom" ? "mt-10 border-y" : "border-y"} border-[#e0cfbf] bg-[#f4e8dc]/75 px-4 py-4 sm:px-6`} aria-label={locale === "en" ? "Mofu Haven service promises" : locale === "ja" ? "Mofu Haven サービスの約束" : "Mofu Haven 服務承諾"}>
