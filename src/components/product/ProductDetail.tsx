  ...
  return <div className="mt-8 space-y-5">
    ...
    {tab === "details" ? <div className="space-y-5">
      <div className="my-6 grid grid-cols-1 items-start gap-6 md:grid-cols-2">
        ...
        {nutrition.length ? <section className="rounded-2xl border border-[#ECE5D8] bg-white p-6 shadow-sm"><h2 className="font-bold">📋 保證營養與規格</h2><dl className="mt-3 divide-y divide-stone-100 text-sm">{nutrition.map((row, index) => <div key={`${row}-${index}`} className="grid grid-cols-[6.5rem_1fr] gap-3 py-2.5"><dt className="font-semibold text-stone-500">{row.includes("：") ? row.slice(0, row.indexOf("：")) : row.includes(":") ? row.slice(0, row.indexOf(":")) : ""}</dt><dd>{row.includes("：") ? row.slice(row.indexOf("：") + 1) : row.includes(":") ? row.slice(row.indexOf(":") + 1) : row}</dd></div>)}</dl></section> : null}
      </div>
