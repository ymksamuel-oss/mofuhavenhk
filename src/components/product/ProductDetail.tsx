...
function RichProductContent({ product, locale, sku, firstImage }: { product: Product; locale: "zh" | "en" | "ja"; sku: string; firstImage: string }) {
  ...
  const nutrition = rich.nutrition.length ? rich.nutrition : productSpecifications(product, sku, locale);
  return <div className="mt-8 space-y-5">
    ...
    {tab === "details" ? <div className="space-y-5">
      <section className="relative overflow-hidden rounded-2xl border border-stone-100 bg-white p-3 shadow-sm sm:p-4"><span className="absolute right-5 top-5 z-10 rounded-full bg-stone-900/70 px-3 py-1 text-[11px] font-bold text-white backdrop-blur">🇯🇵 日本原裝・無添加</span><div className="relative mx-auto my-4 flex h-[300px] w-full max-w-md items-center justify-center overflow-hidden rounded-2xl border border-[#ECE5D8] bg-[#FAF7F2] p-4 sm:h-[360px]"><ProductImage src={firstImage} alt={product.name.zh} sizes="(min-width: 640px) 448px, 100vw" priority className="object-contain mix-blend-multiply" /></div></section>
      ...
      {nutrition.length ? <section className="rounded-2xl border border-stone-200 bg-white p-4"><h2 className="font-bold">📋 保證營養與規格</h2><dl className="mt-3 divide-y divide-stone-100 text-sm">{nutrition.map((row, index) => <div key={`${row}-${index}`} className="grid grid-cols-[6.5rem_1fr] gap-3 py-2.5"><dt className="font-semibold text-stone-500">{row.includes("：") ? row.slice(0, row.indexOf("：")) : row.includes(":") ? row.slice(0, row.indexOf(":")) : ""}</dt><dd>{row.includes("：") ? row.slice(row.indexOf("：") + 1) : row.includes(":") ? row.slice(row.indexOf(":") + 1) : row}</dd></div>)}</dl></section> : null}
