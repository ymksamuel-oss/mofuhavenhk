...
function RichProductContent({ product, locale, sku, firstImage }: { product: Product; locale: Locale; sku: string; firstImage: string }) {
  ...
  return <div className="mt-8 space-y-5">
    ...
    {tab === "details" ? <div className="space-y-5">
      <div className="my-6">
        <section className="relative flex h-[320px] items-center justify-center overflow-hidden rounded-2xl border border-[#ECE5D8] bg-[#FAF7F2] p-6"><span className="absolute right-4 top-4 z-10 rounded-full bg-stone-900/70 px-3 py-1 text-[11px] font-bold text-white backdrop-blur">🇯🇵 {t("badge_japan_pure")}</span><ProductImage src={firstImage} alt={product.name.zh} sizes="(min-width: 768px) 440px, 100vw" priority className="object-contain mix-blend-multiply" /></section>
      </div>
