...
function RichProductContent({ product, sku, firstImage }: { product: Product; sku: string; firstImage: string }) {
  ...
  const shoppingNotes = isZh ? ["📦 香港現貨一般會於 1–2 個工作天內經順豐寄出。", "✈️ 日本預訂及直送商品一般於 7–14 個工作天內抵港；日本公眾假期或會延遲。", "🧺 同一訂單同時包含現貨及預訂／直送商品，將待所有商品備妥後一併發貨。滿 HK$450 享本地免運。", "💬 如對餵食、成分或保存有疑問，歡迎聯絡我們。"] : ["📦 Hong Kong in-stock items are generally shipped within 1–2 business days via SF Express.", "✈️ Japan Pre-order and direct-shipping items usually arrive within 7–14 business days. Japanese public holidays may cause delays.", "🧺 Mixed orders containing in-stock and pre-order items ship together. Local shipping is free for orders over HK$450.", "💬 Contact us if you have questions about feeding, ingredients or storage."];
  ...
  return <div className="mt-8 space-y-5">
    ...
    <div className="flex rounded-xl bg-stone-100 p-1" role="tablist" aria-label={isZh ? "商品內容分頁" : "Product content tabs"}><button type="button" role="tab" aria-selected={tab === "details"} onClick={() => setTab("details")} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-bold ${tab === "details" ? "bg-white text-stone-800 shadow-sm" : "text-stone-500"}`}>{isZh ? "商品詳情" : "Details"}</button><button type="button" role="tab" aria-selected={tab === "notes"} onClick={() => setTab("notes")} className={`flex-1 rounded-lg px-3 py-2.5 text-sm font-bold ${tab === "notes" ? "bg-white text-stone-800 shadow-sm" : "text-stone-500"}`}>{isZh ? "購物須知" : "Shopping Notes"}</button></div>
    {tab === "details" ? <div className="space-y-5">
      <p className="px-1 text-xs text-stone-400">{isZh ? "商品編號：" : "Product code: "}{sku}</p>
      <section className="relative overflow-hidden rounded-2xl border border-stone-100 bg-white p-3 shadow-sm sm:p-4"><span className="absolute right-5 top-5 z-10 rounded-full bg-stone-900/70 px-3 py-1 text-[11px] font-bold text-white backdrop-blur">{isZh ? "🇯🇵 日本製造・無添加" : "🇯🇵 Made in Japan • Additive-Free"}</span><div className="relative aspect-square overflow-hidden rounded-xl bg-[#f5f0e9]"><ProductImage src={firstImage} alt={getLocalizedProductName(product, locale)} sizes="(min-width: 1024px) 700px, 100vw" priority className="object-contain p-3" /></div></section>
      {rich.texture ? <section className="rounded-2xl border border-stone-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><h2 className="font-bold">{isZh ? "🐾 質感與硬度" : "🐾 Texture & hardness"}</h2></div><p className="mt-2 text-sm leading-6 text-stone-600">{rich.texture}</p></section> : null}
      {rich.feeding.length ? <section><h2 className="mb-3 text-lg font-bold">{isZh ? "🍽️ 餵食方法" : "🍽️ Feeding instructions"}</h2><div className="grid gap-3 sm:grid-cols-2">{rich.feeding.map((item, index) => <article key={`${item}-${index}`} className="rounded-2xl border border-stone-200 bg-[#fffdf9] p-4"><p className="text-sm font-bold leading-6">{item}</p></article>)}</div></section> : null}
      <section className="rounded-2xl border border-stone-200 bg-white p-4"><h2 className="font-bold">{isZh ? "📋 營養與規格" : "📋 Nutrition & Specifications"}</h2><dl className="mt-3 divide-y divide-stone-100 text-sm">{nutrition.map((row, index) => { const [key, ...rest] = row.split(":"); return <div key={`${row}-${index}`} className="grid grid-cols-[6.5rem_1fr] gap-3 py-2.5"><dt className="font-semibold text-stone-500">{key}</dt><dd>{rest.join(":") || row}</dd></div>; })}</dl></section>
      {rich.notes.length ? <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><h2 className="font-bold text-amber-900">{isZh ? "💛 溫馨提示" : "💛 Helpful notes"}</h2><ul className="mt-2 space-y-2 text-sm leading-6 text-amber-900/80">{rich.notes.map((note, index) => <li key={`${note}-${index}`}>・{note}</li>)}</ul></section> : null}
    </div> : <section className="rounded-2xl border border-stone-200 bg-white p-5"><h2 className="text-lg font-bold">{isZh ? "🛍️ 購物須知" : "🛍️ Shopping Notes"}</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-stone-600">{shoppingNotes.map((note) => <li key={note}>{note}</li>)}</ul></section>}
  </div>;
}
