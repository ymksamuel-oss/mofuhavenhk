      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mx-auto mb-9 max-w-3xl text-center sm:mb-12">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#806e62] sm:text-base">
            由每日主糧到生活小物，從四個專區開始，慢慢挑選最適合毛孩的日常。
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4">
          {cards.map((card) => (
            <Link
              ...
              aria-label={`前往${card.category.name}`}
            >
              <Image
                src={card.image}
                alt=""
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-6">
                <p className="text-[11px] font-semibold tracking-[0.15em] text-white/80">CURATED FOR EVERY DAY</p>
                <h3 className="mt-2 font-[family-name:var(--font-display)] text-2xl font-semibold tracking-wide sm:text-[1.7rem]">{card.category.name}</h3>
